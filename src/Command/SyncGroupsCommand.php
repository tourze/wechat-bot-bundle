...
class SyncGroupsCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('batch-size', 'b', InputOption::VALUE_REQUIRED, '批次大小', 5)
            ->addOption('delay', 'd', InputOption::VALUE_REQUIRED, '请求间隔（秒）', 2)
...
使用示例：
  php bin/console wechat:sync-groups                      # 同步所有在线账号的群组
  php bin/console wechat:sync-groups 123                 # 同步指定账号的群组
  php bin/console wechat:sync-groups --force             # 强制同步所有账号
  php bin/console wechat:sync-groups --only-online       # 只同步在线账号
  php bin/console wechat:sync-groups --sync-members      # 同时同步群成员信息
  php bin/console wechat:sync-groups --batch-size=3      # 设置批次大小
');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $syncMembers = $input->getOption('sync-members');
    ...
    private function syncGroupMembers(WeChatAccount $account, int $delay): void
    {
        try {
            // 获取账号下的所有群组
            $groups = $this->entityManager->getRepository(WeChatGroup::class)
                ->findBy(['account' => $account, 'inGroup' => true]);
